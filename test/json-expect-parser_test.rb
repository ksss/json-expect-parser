...
module JSONExpectParserTest
  ...
  def test_parse_error(t)
    [
      ...
      ["[}", ->(e) { e.array {} }, /nothing expectation in block/],
      [%({), ->(e) { e.object {} }, /expected "}" or """ but was nil/],
      [%({"a":1), ->(e) { e.object { e.key; e.integer } }, /expected "}" or """ but was nil/],
      [%({"a":1,), ->(e) { e.object { e.key; e.integer } }, /expected next token but was EOF/],
